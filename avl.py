...
def manual_button():
    ...
    EntryID.bind('<Button-1>', clearId)  # Limpa o texto quando se clica (campo valor)
    EntryID.pack()  # gerenciador de geometria
    EntryName.bind('<Button-1>', clearName)  # Limpa o texto quando se digita(campo nome)
    EntryName.pack()
    EntryID.focus_set()  # obtem o foco para a entrada de texto
    EntryName.focus_set()
    insert_btn = Button(ManualInsert, text='Inserir do Arquivo', width=20, command=inserir_btn_func) #botão de inserir
    insert_btn.pack()


def clearId(event):  # fncao para limpar text field
    EntryID.delete('0', END)

def clearName(event):   # fncao  para limpar text field
...
def remover_button():
    ...
    entry.focus_set()  # obtem o foco para a entrada de texto
    global arvore
    janelaRemover = Tk()  # cria uma janela
    janelaRemover.title('AVL')  # seta o titulo da janela
    janelaRemover.geometry('200x50')  # seta o tamanho da janela
...
def localizar_button():
    global localizado
    texFieldLocaliza = Entry(window, width=25, justify='center')  # cria uma entrada de texto
    texFieldLocaliza.insert(0, '')  # seta o texto
    texFieldLocaliza.pack()  # gerenciador de geometria
    texFieldLocaliza.focus_set()  # obtem o foco para a entrada de texto

    if not texFieldLocaliza.get():  # [] entrada vazia
        texFieldLocaliza.insert(0, 'Digite o Nó')
        texFieldLocaliza.bind('<Button-1>', clearText)
    else:
